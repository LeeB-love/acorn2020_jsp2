...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	//GET 방식 파라미터로 전달되는 다운로드 시켜줄 파일의 번호를 읽어온다.
	int num = Integer.parseInt(request.getParameter("num"));

	//DB에서 해당 파일의 정보를 select 해온다.
	FileDto dto = FileDao.getInstance().getData(num);
	
	//해당 파일을 실제로 다운로드 시켜준다.(응답) ===> 복붙해오기
	
	//다운로드 시켜주기 ... 
	String orgFileName=dto.getOrgFileName(); //원본 파일명
	String saveFileName=dto.getSaveFileName(); //저장된 파일명
	
	//다운로드 시켜줄 파일의 실제 경로 구성하기 ==> 궁금하면 System.out.println() 이용해서 출력해보기
